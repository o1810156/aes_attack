...
#[allow(dead_code)]
fn sbox(val: GF256) -> GF256 {
    GF256::new(SBOX[val.get_u8() as usize])
}

#[allow(dead_code)]
fn sbox_u8(val: u8) -> u8 {
    SBOX[val as usize]
}
    ...
    for &(u0, u1) in up_cands.iter() {
        for &(d0, d1) in down_cands.iter() {
            ...
            for (i, diff_cipher) in diff_ciphers.iter().enumerate() {
                ...
                let v = a / coef[0];
                let prop = ddts[i][v.get_u8() as usize];
                ...
                if prop == 0 {
                    println!("This is impossible difference.");
                    flag = false;
                    break;
                }
...
fn kr_sub(
    ...
    for (i, partial_key_cand) in partial_keys.iter().enumerate() {
        let partial_key = &partial_key_cand[select[i]].array;
...
fn make_double_ddt(delta: usize, coef: GF256) -> Vec<usize> {
    let ddt = ddt::sbox_ddt();
    let len = ddt.len();

    let mut res = vec![0; len];
    for i in 0..len {
        let first_prop = ddt[delta][i];
        ...
        for j in 0..len {
            let t = GF256::new(i as u8) * coef;
            let next_prop = ddt[t.get_usize()][j];

            if next_prop == 0 {
                continue;
            }

            res[j] += first_prop * next_prop;
        }
    }
...
fn key_recovery_attack_using_differential_cryptanalysis(query: u8, verbose: bool) -> Result<bool> {
    ...
    let mut ddts = Vec::new();
    let mut ddts2 = Vec::new();
    let mut ddts3 = Vec::new();
    ...
    for i in 1_u8..=query {
        ...
        let ddt = make_double_ddt(i as _, GF256::new(1));
        ddts.push(ddt);
        let ddt2 = make_double_ddt(i as _, GF256::new(2));
        ddts2.push(ddt2);
        let ddt3 = make_double_ddt(i as _, GF256::new(3));
        ddts3.push(ddt3);
    ...
    if verbose {
        println!("Partial Key Guess Start: 1");
    }
    let partial_key_col1 = partial_key_guess(
        &original_cipher,
        &diff_ciphers,
        &key_indices[0],
        &coefs[0],
    ...
    if verbose {
        println!("Cands num @ [0] : {}", partial_key_col1.len());
        println!("Top Cand Score: {}", partial_key_col1[0].weight);
    }

    if verbose {
        println!("Partial Key Guess Start: 2");
    }
    let partial_key_col2 = partial_key_guess(
        &original_cipher,
        &diff_ciphers,
        &key_indices[1],
        &coefs[1],
    ...
    if verbose {
        println!("Cands num @ [1] : {}", partial_key_col2.len());
        println!("Top Cand Score: {}", partial_key_col2[0].weight);
    }

    if verbose {
        println!("Partial Key Guess Start: 3");
    }
    let partial_key_col3 = partial_key_guess(
        &original_cipher,
        &diff_ciphers,
        &key_indices[2],
        &coefs[2],
    ...
    if verbose {
        println!("Cands num @ [2] : {}", partial_key_col3.len());
        println!("Top Cand Score: {}", partial_key_col3[0].weight);
    }

    if verbose {
        println!("Partial Key Guess Start: 4");
    }
    let partial_key_col4 = partial_key_guess(
        &original_cipher,
        &diff_ciphers,
        &key_indices[3],
        &coefs[3],
    ...
    if verbose {
        println!("Cands num @ [3] : {}", partial_key_col4.len());
        println!("Top Cand Score: {}", partial_key_col4[0].weight);
    }
