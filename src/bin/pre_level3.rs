...
fn partial_key_guess(
    ...
    for i in 0_u8..=255 {
        for j in 0_u8..=255 {
            ...
            let a = inv_sbox(original[0] ^ i) ^ inv_sbox(diff[0] ^ i);
            let b = inv_sbox(original[1] ^ j) ^ inv_sbox(diff[1] ^ j);
            let c = inv_sbox(original[2] ^ i) ^ inv_sbox(diff[2] ^ i);
            let d = inv_sbox(original[3] ^ j) ^ inv_sbox(diff[3] ^ j);
    ...
    for &(u0, u1) in up_cands.iter() {
        for &(d0, d1) in down_cands.iter() {
            let a = inv_sbox(original[0] ^ u0) ^ inv_sbox(diff[0] ^ u0);
            let b = inv_sbox(original[1] ^ u1) ^ inv_sbox(diff[1] ^ u1);
            let c = inv_sbox(original[2] ^ d0) ^ inv_sbox(diff[2] ^ d0);
            let d = inv_sbox(original[3] ^ d1) ^ inv_sbox(diff[3] ^ d1);

            /*
            if half_judge_middle(&[b, b, c, c], coef) {
...
fn key_recovery_attack_using_differential_cryptanalysis(verbose: bool) -> Result<bool> {
    ...
    for index in plain_indices.iter() {
    ...
    if verbose {
        for i in 0..4 {
            println!("Cands num @ [{}] : {}", i, partial_keys[i].len());
        }
    }
...
fn main() -> Result<()> {
    let verbose = true;
    let result = key_recovery_attack_using_differential_cryptanalysis(verbose)?;
    println!("{}", result);
